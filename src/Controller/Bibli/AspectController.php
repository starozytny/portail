...
class AspectController
{
    ...
    private function submitForm($request, $type, $id): array
    {
        ...
        if($type == "create"){
            $res = $this->apiService->callApiWithErrors('library/add_aspect/', 'POST', false, $dataToSend);
        }else{
            $res = $this->apiService->callApiWithErrors('library/edit_aspect/' . $id, 'POST', false, $dataToSend);
        }

        if($res['code'] == 0 && str_contains($res['data'], "already")){
            return ['code' => 0, 'data' => json_encode([['name' => 'name', 'message' => "Cet aspect existe déjà."]])];
        }
    ...
    {
        return $this->dataService->delete($response, 'library/delete_aspect/' . $args['id'], "GET");
    }
}